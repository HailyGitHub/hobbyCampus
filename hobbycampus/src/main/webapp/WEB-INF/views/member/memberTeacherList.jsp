	...
	<main class="main-area">
		...
		<article class="select-list m-5">
			<div class="d-grid gap-2 d-md-block col-md-12 text-center">
				...
				<button class="btn btn-primary btn-lg" type="button" id="btn_2"><i class="bi bi-easel-fill"></i> 강사계정</button>
				<button class="btn btn-outline-primary btn-lg" type="button" id="btn_3"><i class="bi bi-person-dash-fill"></i> 탈퇴계정</button>
			</div>
		</article>
				...
				<table class="table m-4">
					<thead>
						<tr>
							...
							<th scope="col" class="col-md-2">레벨</th>
					...
					<tbody>
						...
						<c:forEach var="dto" items="${lists}">
							<tr>
								<th scope="row">${dto.t_idx}</th>
								<td><img src="/hobbycampus/hobbyImg/teacherImg/${dto.t_img}" class="rounded-circle" data-img="${dto.t_img}"></td>
								<td>${dto.u_email}</td>
								<td>${dto.t_name}</td>
								<td>${dto.lev_name}</td>
								<td>${dto.t_point}</td>
								<td>
									<div class="form-check form-switch">
										<input class="form-check-input" type="checkbox" role="switch" onclick="return switchActive(${dto.u_idx})" id="${dto.u_idx}" data-state="${dto.u_state}" ${dto.u_state=='활성'? 'checked':''}>
