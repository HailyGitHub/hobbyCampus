$myLesson = {
	...
	applyCoupon: function(e) {
		...
		$myLesson.paymentPrice = $myLesson.totalPrice - couponPrice;
		document.getElementById('total_price_hr').innerText = "총 결제 포인트 : " + Math.max(0, $myLesson.paymentPrice).toString();
	},
	payment: function() {
		//먼저 배송지가 있는 경우 비어있는거 체크
		if (!$myLesson.validate()) {
			return;
		}

		//먼저 포인트가 충분한지 확인
		if ($myLesson.uPoint < $myLesson.paymentPrice) {
			//충전 confirm 창 생성
			if (confirm("충전하시겠습니까?")) {
				//포인트 부족
				var url = "/hobbycampus/pointShop.do?rurl=" + encodeURIComponent(location.href);
				//기존에 주소가 저장되어있지 않았음.
				if ($myLesson.postSaved == "N") {
			...
			url += "?lessonScheduleIdx=" + $myLesson.lessonScheduleIdx;
			url += "&pricePoint=" + $myLesson.paymentPrice;
